...
export function CrashAlertOverlay() {
  ...
  useEffect(() => {
    ...
    // Auto-fire outbound message to contacts.
    triggerContacts();
    setSent(true);
  ...
  function triggerContacts() {
    ...
    const msg = `[VIGLA] Une chute a été détectée. Position: ${lat ?? "?"}, ${lng ?? "?"} — ${mapLink}`;
    for (const c of contacts) {
      if (c.phone) {
        window.open(`sms:${c.phone}?body=${encodeURIComponent(msg)}`, "_blank");
      } else if (c.email) {
        window.open(
          `mailto:${c.email}?subject=${encodeURIComponent("[VIGLA] Alerte accident")}&body=${encodeURIComponent(msg)}`,
          "_blank",
        );
      }
    }
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex flex-col items-center justify-center bg-red-600/95 p-6 text-white backdrop-blur">
      ...
      <div className="text-lg font-semibold uppercase tracking-widest">Accident détecté</div>
      <div className="mt-1 text-sm text-white/80">
        {sent ? "Message envoyé aux contacts" : "Confirmez que vous allez bien"}
      </div>
      ...
      <div className="relative my-8 h-48 w-48">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <div className="text-xs uppercase tracking-widest text-white/70">secondes</div>
        </div>
      </div>
      ...
      <div className="w-full max-w-sm space-y-3">
        ...
        >
          <Phone className="h-5 w-5" /> J'appelle le 112
        </a>
        <button
        ...
        >
          <Check className="h-5 w-5" /> Je vais bien
        </button>
        {contacts.length === 0 && (
          <p className="text-center text-xs text-white/70">
            Aucun contact d'urgence enregistré. Ajoutez-en dans Profil.
          </p>
        )}
      </div>
    </div>
